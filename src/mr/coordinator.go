...
func (c *Coordinator) ApplyForTask(args *ApplyTaskArgs, reply *Task) error {
	mutex.Lock()
	defer mutex.Unlock()
	if c.State == Map {
		if len(c.MapChan) > 0 {
...
func (c *Coordinator) FinishTask(task *Task, reply *ApplyTaskReply) error {
...
func (c *Coordinator) Example(args *ApplyTaskArgs, reply *ApplyTaskReply) error {
	//reply.Y = args.X + 1
	return nil
}
